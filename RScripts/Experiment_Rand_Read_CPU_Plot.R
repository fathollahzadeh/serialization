...
drawPlot <- function(data,isseq,ts,hasshleg){
  ...
  axis(1, at=c(1, 2, 3, 4,5), labels=c("1M","2M","3M", "4M","5M"), las=1,cex.axis = 0.5 ,mgp=c(3, .6, 0))
  ...
  title(xlab=xlb, col.lab="black",family="Helvetica",cex.lab = 0.6,line = 1.5,font=2)
    ...
    if(ts=="true"){
      title(ylab="Total Random Read Time (sec) - log", col.lab="black",family="Helvetica",cex.lab = 0.6,line =1.7,font=2)
    }
  
  box()
  
  if(ts=="true"){
    legend(3.1, 19, serialization_methods[c(1:6)], cex=0.3,  col=plot_colors[c(1:6)], pch=1:6, lty=2:2,lwd=0.5,ncol=1);
  }
  else{
    legend(3.1, 20, serialization_methods[c(7:13)], cex=0.3,  col=plot_colors[c(7:13)], pch=7:13, lty=2:2,lwd=0.5,ncol=1);
  }
}
...
pdf(file='Experiment_Rand_Read_CPU_Plot.pdf',height=3.5, width=4)
