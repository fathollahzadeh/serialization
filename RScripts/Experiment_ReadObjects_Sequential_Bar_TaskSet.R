...
data <- rbind(cppHandCoded,cppInPlace,cppBoost,cppProtoBuf,cppBson,cppFlatBuf,javaDefault,javaJson,javaBson,javaKryo,javaByteBuffer,javaProtoBuf,javaFlatBuf)
...
# draw bar plots
p1 <- barplot(valuestasksettrue, log="y", col=colorstasksettrue, ylim = c(min_y,  max(max_y,na.rm = TRUE)+max_y/2), legend.text=TRUE, axes=FALSE,border ="gray" , space=c(0.2,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5))

p2 <- barplot(valuestasksetfalse, log="y", col=colorstasksetfalse, ylim = c(min_y,  max(max_y,na.rm = TRUE)+max_y/2), add=T,border ="gray", space=c(1.4,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5,2.5),axes=FALSE)
...
text(x=p[c(-1,-2,-3,-4,-26)]+0.2, y=totaltime[c(-1,-2,-3,-4,-26)]-totaltime[c(-1,-2,-3,-4,-26)]*0.19, font = 1, font.lab = 2, labels=round(cputime[c(-1,-2,-3,-4,-26)], 1), pos=3, xpd=NA, cex=0.6,srt=90)
text(x=p[26]+0.2, y=totaltime[26]-totaltime[26]*0.16, font = 1, font.lab = 2, labels=round(cputime[26], 1), pos=3, xpd=NA, cex=0.45,srt=90)
text(x=p+0.2, y=iotime-iotime*0.2, font = 1, font.lab = 2, labels=round(iotime, 1), pos=3, xpd=NA, cex=0.6,srt=90)
box()

lines(x=p[1]-2, y=min_y-min_y/5.5,lwd = 1, col = "grey")
text(x=(p1+p2)/2-2,  y=min_y-min_y/3.1, xnamestasksettrue, xpd=NA, srt=45, pos=3, font=1, cex=0.7)

legend("topright", regions, cex = 0.8, fill = regionscolors)

mtext("Total Reading Time (sec) - log", outer = TRUE, cex = 1, font=0.7,side=2,family="Helvetica",line = -0.1, col=rgb(0,0.5,0) )
mtext("Sequential Read Objects for 5M Tweets", outer = TRUE, cex = 1, font=0.7,side=1,family="Helvetica" ,line = -0.1,col=rgb(0,0.5,0))
