...
pdf(file='Experiment_External_Sort_CPU_IO_Bar_10M.pdf',height=2, width=3)
...
data <- rbind(cppInPlace,cppHandCoded,cppFlatBuf,cppProtoBuf,cppBoost,cppBson,javaJson,javaDefault,javaBson,javaKryo,javaProtoBuf,javaByteBuffer,javaFlatBuf)
...
text(x=p+0.05, y=totaltime-totaltime*0.5, font = 1, font.lab = 2, labels=round(cputime, 1), pos=3, xpd=NA, cex=0.25,srt=0)
text(x=p[-7]+0.05, y=iotime[-7]-iotime[-7]*0.5, font = 1, font.lab = 2, labels=round(iotime[-7], 1), pos=3, xpd=NA, cex=0.25,srt=0,col = "white")
text(x=p[7]+0.05, y=iotime[7]-iotime[7]*0.45, font = 1, font.lab = 2, labels=round(iotime[7], 1), pos=3, xpd=NA, cex=0.25,srt=0,col = "white")
box()

lines(x=p[1]-2, y=min_y-min_y/5.5,lwd = 1, col = "grey")
text(x=p+0.9,  y=min_y-0.17, xnamestasksettrue, xpd=NA, srt=30, pos=2, font=1, cex=0.4)
...
title(ylab="Total Sort Time(minute) - log", col.lab="black",family="Helvetica",cex.lab = 0.6,line = 1.3)
