...
min_y<-min(totaltime)/2 #max(min(totaltime)-min(totaltime)/2,50)
...
text(x=p1[-2], y=totaltime[-2]-totaltime[-2]*0.15, font = 1, font.lab = 2, labels=round(cputime[-2], 1), pos=3, xpd=NA, cex=0.7)
text(x=p1, y=iotime-iotime*0.15, font = 1, font.lab = 2, labels=round(iotime, 1), pos=3, xpd=NA, cex=0.7)
...
box()
text(x=p1-0.7,  y=min_y-27, xnames, xpd=NA, srt=45, pos=3,  cex=0.9)
...
title(xlab = "Random Read Objects for 4M Tweets",  cex.lab = 1.2, line = 5, cex=1, font=2)
