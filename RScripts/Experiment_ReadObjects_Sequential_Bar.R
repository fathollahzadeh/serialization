...
min_y<-max(min(totaltime)-100,50)
#tick_list=c(min_y,0.3,0.6,1,2,3, 8,15, 25, 45,  80, 110, 200,370,570,800,1200,max_y)


p1 <- barplot(values, log="y", col=colors, ylim = c(min_y,  max(totaltime,na.rm = TRUE)+1800), legend.text=TRUE, axes=FALSE)

z <-c(min_y,80,148,173,265,302,509,649,1307,1485,1940)
...
box()
text(x=p1-0.7,  y=min_y-20, labs, xpd=NA, srt=45, pos=3,  cex=1)
legend("topleft", regions, cex = 1, fill = colors)
title(ylab = "Time (second)- log",  cex.lab = 1.2, line = 2.5, cex=1, font=1,col.lab=rgb(0,0.5,0))
title(xlab = "Sequential Read Objects for 4M Tweets",  cex.lab = 1.2, line = 5.3, cex=1, font=1,col.lab=rgb(0,0.5,0))
