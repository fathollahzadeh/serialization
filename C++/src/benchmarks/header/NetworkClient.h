		...
		try {
			...
			try {
				...
				while (true) {
					...
					// if io type is RAM:
					if (ioType == 1) {
						startIndex = fileHandler->getNextNetworkPage(objectList, startIndex, this->numOfObjects,
																	 longBuffer);
					}
						// if io type is Disk:
					else if (ioType == 2) {
					...
					// break Out: Done:
					if ((this->ioType == 1 && startIndex >= this->numOfObjects) ||
						(this->ioType == 2 && (localPageCount >= totalPageCount || pageSize == 0))) {
						break;
					}
				}
