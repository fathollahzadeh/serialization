...
void ExternalSort<T>::runTheExternalSort() {
	...
	priority_queue < ObjectFileIndex<T> * , vector < ObjectFileIndex<T> * >, UniversalPointerComparatorDescending<T> >
																			 queue;
...
ExternalSort<T>::ExternalSort(string fileName, int numberOfFiles, int serializationType, int round, bool taskset) {
	...
	this->logFileHandler = new LogFileHandler(
			"bin/benchmark/externalsort/result_cpp_externalsort_" + to_string(round) + ".txt");
}
