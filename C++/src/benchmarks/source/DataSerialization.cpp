...
int main(int argc, char *argv[]) {
	...
	switch (serializationType) {
		...
		case 3:
		case 5: {
			DataSerialization <TweetStatus> *dataSerialization = new DataSerialization<TweetStatus>(inputFile, outFile,
																									serializationType,
																									numberOfTweets,
																									round, taskset);
		...
		case 2: {
			DataSerialization <TweetStatusIP> *dataSerialization = new DataSerialization<TweetStatusIP>(inputFile,
																										outFile,
																										serializationType,
																										numberOfTweets,
																										round, taskset);
		...
		case 4: {
			DataSerialization <TweetStatusProto> *dataSerialization = new DataSerialization<TweetStatusProto>(inputFile,
																											  outFile,
																											  serializationType,
																											  numberOfTweets,
																											  round,
		...
		case 6: {
			DataSerialization <TweetStatusFlatBuffers> *dataSerialization = new DataSerialization<TweetStatusFlatBuffers>(
					inputFile, outFile, serializationType, numberOfTweets, round, taskset);
			dataSerialization->runTheDataSerialization();
