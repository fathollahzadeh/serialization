...
TweetStatus* DataReader::generateTweetObject(Document &d) {
    ...
    return new TweetStatus(createdAt, id, text, source, isTruncated, inReplyToStatusId, inReplyToUserId,
                            ...
                            extendedEntities, isFavorited, isRetweeted, isPossiblySensitive, filterLevel,
...
User* DataReader::getUser(Value &d) {
   ...
   return new User(id, name, screenName, location, url,
    ...
    for (SizeType i = 0; i < d.Size(); i++) {
        ...
        if (d[i].HasMember("video_info") && !d[i]["video_info"].IsNull()) {
            ...
            for (SizeType j = 0; j < d[i]["video_info"]["variants"].Size(); j++) {
                ...
                !d[i]["video_info"]["variants"][j]["url"].IsNull()?d[i]["video_info"]["variants"][j]["url"].GetString():"";
        ...
        MediaEntity *mediaEntity = new MediaEntity(displayURL, expandedURL, id, indices, mediaURL, mediaURLHttps,
