...
TweetStatusProto::TweetStatusProto(TweetStatus *tweetStatus) {
    ...
    // set user:
    proto::TweetStatusP_UserP *user = proto->mutable_userp();
    ...
    if (tweetStatus->coordinates != nullptr) {
        ...
        proto::TweetStatusP_CoordinatesP *coordinatesP=proto->mutable_coordinates();
    ...
    if (tweetStatus->place!= nullptr) {
        proto::TweetStatusP_PlaceP *placeP = proto->mutable_place();
        ...
        if (tweetStatus->place->boundingBoxCoordinates!= nullptr){
            proto::TweetStatusP_PlaceP_BoundingBoxCoordinateP *boundingBoxCoordinateP=placeP->mutable_boundingboxcoordinatep();
            ...
            for (int i = 0; i <tweetStatus->place->boundingBoxCoordinates->coordinates.size() ; ++i) {
                proto::TweetStatusP_PlaceP_BoundingBoxCoordinateP_RowsOneP *rowsOneP=boundingBoxCoordinateP->add_threedimarray();
                for (int j = 0; j <tweetStatus->place->boundingBoxCoordinates->coordinates.at(i).size() ; ++j) {
                    proto::TweetStatusP_PlaceP_BoundingBoxCoordinateP_RowsOneP_RowsTwoP *rowsTwoP=rowsOneP->add_rowstwop();

                    for (int k = 0; k <tweetStatus->place->boundingBoxCoordinates->coordinates.at(i).at(j).size() ; ++k) {
                        rowsTwoP->add_coordinate(tweetStatus->place->boundingBoxCoordinates->coordinates.at(i).at(j).at(k));
                    }
                }
            }
        }
    ...
    if (tweetStatus->entities!= nullptr){
        ...
        proto::TweetStatusP_EntitiesP *entitiesP=this->proto->mutable_entitiesp();
        //HashtagEntity
        for (int i = 0; i <tweetStatus->entities->hashtags.size() ; ++i) {
            proto::HashtagEntityP *hashtagEntityP=entitiesP->add_hashtagentityp();
            hashtagEntityP->set_text(tweetStatus->entities->hashtags.at(i)->text);

            for (int j = 0; j <tweetStatus->entities->hashtags.at(i)->indices.size() ; ++j) {
                hashtagEntityP->add_indices(tweetStatus->entities->hashtags.at(i)->indices.at(j));
            }
        }

        //MediaEntity
        for (int i = 0; i <tweetStatus->entities->media.size() ; ++i) {
            proto::MediaEntityP *mediaEntityP=entitiesP->add_mediaentityp();
            this->setMediaEntityP(mediaEntityP,tweetStatus->extendedEntities->media.at(i));
        }
        
        //URLEntity
        for (int i = 0; i < tweetStatus->entities->urls.size(); ++i) {
            proto::URLEntityP *urlEntityP = entitiesP->add_urlentityp();
            urlEntityP->set_displayurl(tweetStatus->entities->urls.at(i)->displayURL);
            urlEntityP->set_url(tweetStatus->entities->urls.at(i)->url);
            urlEntityP->set_expandedurl(tweetStatus->entities->urls.at(i)->expandedURL);

            for (int j = 0; j < tweetStatus->entities->urls.at(i)->indices.size(); ++j) {
                urlEntityP->add_indices(tweetStatus->entities->urls.at(i)->indices.at(j));
            }
        }
        //UserMentionEntity
        for (int i = 0; i <tweetStatus->entities->userMentions.size() ; ++i) {
            proto::UserMentionEntityP *userMentionEntityP=entitiesP->add_usermentionentityp();
            userMentionEntityP->set_id(tweetStatus->entities->userMentions.at(i)->id);
            userMentionEntityP->set_idstr(tweetStatus->entities->userMentions.at(i)->idStr);

            for (int j = 0; j <tweetStatus->entities->userMentions.at(i)->indices.size() ; ++j) {
                userMentionEntityP->add_indices(tweetStatus->entities->userMentions.at(i)->indices.at(j));
            }
            userMentionEntityP->set_name(tweetStatus->entities->userMentions.at(i)->name);
            userMentionEntityP->set_screenname(tweetStatus->entities->userMentions.at(i)->screenName);
            
        }

        //SymbolEntity
        for (int i = 0; i <tweetStatus->entities->symbols.size() ; ++i) {
            proto::SymbolEntityP *symbolEntityP=entitiesP->add_symbolentityp();
            symbolEntityP->set_text(tweetStatus->entities->symbols.at(i)->text);

            for (int j = 0; j <tweetStatus->entities->symbols.at(i)->indices.size() ; ++j) {
                symbolEntityP->add_indices(tweetStatus->entities->symbols.at(i)->indices.at(j));
            }
        }

        //PollEntity
        for (int i = 0; i <tweetStatus->entities->polls.size() ; ++i) {
            proto::PollEntityP *pollEntityP=entitiesP->add_pollentityp();
            pollEntityP->set_enddatetime(tweetStatus->entities->polls.at(i)->endDatetime);
            pollEntityP->set_durationminutes(tweetStatus->entities->polls.at(i)->durationMinutes);

            for (int j = 0; j <tweetStatus->entities->polls.at(i)->options.size() ; ++j) {
                proto::OptionEntityP *optionEntityP=pollEntityP->add_options();
                optionEntityP->set_position(tweetStatus->entities->polls.at(i)->options.at(j)->position);
                optionEntityP->set_text(tweetStatus->entities->polls.at(i)->options.at(j)->text);
            }
        }
    }
    if(tweetStatus->extendedEntities!= nullptr){
        proto::TweetStatusP_ExtendedEntitiesP *extendedEntitiesP=this->proto->mutable_extendedentities();

        for (int i = 0; i <tweetStatus->extendedEntities->media.size() ; ++i) {
